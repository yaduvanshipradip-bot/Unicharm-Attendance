...
export default function App() {
  ...
  const [showHrResetPass, setShowHrResetPass] = useState(false);
  ...
  // HR Form: Department -> Emp ID -> Name -> Password
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      alert('Firebase connect nahi ho pa raha. Internet ya rules check karo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // HR CREATES EMPLOYEE (1. Dept -> 2. EmpID -> 3. Name -> 4. Password)
  const saveEmployee = async () => {
    ...
    if (!cleanDept || !cleanEmpId || !cleanName || !cleanPass) {
      return alert('Department, Employee ID, Name aur Password sabhi bharna zaroori hai!');
    }
  ...
  const handleHrResetEmpPassword = async () => {
    ...
    try {
      ...
      alert(`Success! Employee (${empExists.name}) ka password change ho gaya: ${cleanPass}`);
      setHrResetEmpId('');
      setHrResetEmpPass('');
      setHrPage('home');
    } catch (err) {
  ...
  const dynamicCardStyle: React.CSSProperties = {
    ...
    border: role === 'login' ? '2px solid rgba(255, 255, 255, 0.2)' : '1px solid #cbd5e1',
  };
                ...
                <div style={{ padding: '20px 0' }}>
                  {hrPage === 'home' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      ...
                      {/* NEW: RESET EMPLOYEE PASSWORD BUTTON */}
                      <button
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        <button
                          ...
                          style={styles.eyeBtn}
                        >
                          {showEmpPass ? <EyeOff size={18} /> : <Eye size={18} />}
                        </button>
                      </div>
                  ...
                  {hrPage === 'resetEmpPass' && (
                    <div style={{ textAlign: 'left' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          type={showHrResetPass ? 'text' : 'password'}
                        ...
                        <button
                          type="button"
                          onClick={() => setShowHrResetPass(!showHrResetPass)}
                          style={styles.eyeBtn}
                        >
                          {showHrResetPass ? <EyeOff size={18} /> : <Eye size={18} />}
                        </button>
                      </div>
